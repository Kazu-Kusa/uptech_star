...
class PickUpBall:
    ...
    def update_frame(self, img, h_min, h_max, s_min, s_max, v_min, v_max):
        ...
        if len(cnts) >= 1 and self.is_detect:
            ...
            self.up.set_controller_cmd(UpController.MOVE_STOP)
...
if __name__ == '__main__':
    ...
    cv2.createTrackbar("S_MIN", "img", 122, 255, nothing)
    cv2.createTrackbar("S_MAX", "img", 128, 255, nothing)
    cv2.createTrackbar("V_MIN", "img", 170, 255, nothing)
    cv2.createTrackbar("V_MAX", "img", 180, 255, nothing)
